...
impl Default for SigningApp {
    fn default() -> Self {
        Self {
            ...
            status: StatusMessage {
                text: "Welcome to IronVault Transaction Signing".to_string(),
...
impl SigningApp {
    ...
    fn draw_title(&self, f: &mut Frame, area: Rect) {
        ...
        let title = format!("IronVault Transaction Signing - {}", wallet_status);
    ...
    fn prepare_export(&mut self) {
        if let Some(ref psbt_data) = self.current_psbt {
            ...
            match generator.generate_parts(psbt_data, ironvault_qr::FileType::Transaction) {
                Ok(qr_codes) => {
