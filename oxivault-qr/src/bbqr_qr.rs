...
use crate::{Result, bbqr::{BBQrEncoder, BBQrDecoder, FileType, EncodingType}};
...
impl BBQrQrGenerator {
    ...
    fn generate_qr_for_part(&self, part_data: &str) -> Result<QrCode> {
        if let Some(version) = self.preferred_version {
            ...
            QrCode::with_version(part_data, min_version, self.ec_level)
                .or_else(|_| {
...
mod tests {
    ...
    fn test_encoding_selection() {
        ...
        assert_eq!(generator.select_encoding_type(text_data), EncodingType::Base32);
