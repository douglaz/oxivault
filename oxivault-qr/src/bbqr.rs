...
impl BBQrHeader {
    ...
    pub fn from_string(s: &str) -> Result<Self> {
        ...
        // Parse hex values
        let total_parts = u16::from_str_radix(&s[5..9], 16).map_err(|_| "Invalid total parts")?;
        let part_number = u16::from_str_radix(&s[9..13], 16).map_err(|_| "Invalid part number")?;
        let data_length = u32::from_str_radix(&s[13..21], 16).map_err(|_| "Invalid data length")?;
        let checksum_val = u32::from_str_radix(&s[21..29], 16).map_err(|_| "Invalid checksum")?;
        ...
        match encoding_type {
            EncodingType::Raw => {
                ...
                for chunk in data.as_bytes().chunks(2) {
                    if chunk.len() == 2 {
                        let byte = u8::from_str_radix(&String::from_utf8_lossy(chunk), 16)
