...
impl PlayingCard {
    ...
    pub fn from_str(s: &str) -> Result<Self> {
        ...
        if suit_idx >= chars.len() {
            return Err(Error::InvalidEntropy("Missing suit in card notation".into()));
        }
...
impl EntropyGenerator {
    ...
    pub fn mix_with_system_rng(user_entropy: &[u8]) -> Result<Vec<u8>> {
        use crate::rng::{EntropyMixer, SecureRng, RandomSource};
    ...
    pub fn to_mnemonic_entropy(entropy: &[u8], word_count: usize) -> Result<Vec<u8>> {
        let entropy_bytes = match word_count {
            ...
            18 => 24,
            24 => 32,
            _ => return Err(Error::InvalidEntropy("Invalid word count (use 12, 18, or 24)".into())),
        };
...
mod tests {
    ...
    fn test_coin_entropy() -> Result<()> {
        ...
        let mut flips = Vec::new();
        for i in 0..256 {
            flips.push(if i % 2 == 0 { CoinFlip::Heads } else { CoinFlip::Tails });
        }
