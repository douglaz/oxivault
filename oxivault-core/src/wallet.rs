...
use crate::bip32::{HdWallet, DerivationPaths};
...
impl Wallet {
    ...
    pub fn get_address(
        ...
        let compressed_pk = bitcoin::key::CompressedPublicKey::from_private_key(&secp, &private_key.to_priv()).unwrap();
    ...
    pub fn get_account_xpub(&self, script_type: ScriptType, account: u32) -> Result<Xpub> {
        self.hd_wallet.account_xpub(script_type.bip_number(), account)
    }
...
impl MultisigWallet {
    ...
    pub fn get_address(&self, change: u32, index: u32) -> Result<Address> {
        ...
        use bitcoin::opcodes::all::{OP_CHECKMULTISIG, OP_PUSHNUM_1, OP_PUSHNUM_2, OP_PUSHNUM_3, OP_PUSHNUM_4, OP_PUSHNUM_5, OP_PUSHNUM_6, OP_PUSHNUM_7, OP_PUSHNUM_8, OP_PUSHNUM_9, OP_PUSHNUM_10, OP_PUSHNUM_11, OP_PUSHNUM_12, OP_PUSHNUM_13, OP_PUSHNUM_14, OP_PUSHNUM_15};
        ...
        for xpub in &self.xpubs {
            ...
            let child_path = [
                ChildNumber::from_normal_idx(change).map_err(|_| Error::InvalidDerivationPath(format!("Invalid change index: {}", change)))?,
                ChildNumber::from_normal_idx(index).map_err(|_| Error::InvalidDerivationPath(format!("Invalid index: {}", index)))?
            ];
            
            let derived_xpub = xpub.derive_pub(&secp, &child_path)
        ...
        builder = match self.threshold {
            ...
            _ => return Err(Error::BitcoinError("Invalid threshold (must be 1-15)".into())),
        };
        ...
        builder = match pubkeys.len() {
            ...
            14 => builder.push_opcode(OP_PUSHNUM_14),
            15 => builder.push_opcode(OP_PUSHNUM_15),
            _ => return Err(Error::BitcoinError("Invalid number of pubkeys (must be 1-15)".into())),
        };
...
mod tests {
    ...
    fn test_address_generation() {
        ...
        let address = wallet.get_address(ScriptType::NativeSegwit, 0, 0, 0).unwrap();
