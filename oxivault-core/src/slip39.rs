...
impl Share {
    ...
    pub fn to_mnemonic(&self) -> Result<String> {
        ...
        let mnemonic: Vec<&str> = words.iter()
            .map(|&idx| SLIP39_WORDLIST.get(idx as usize)
                .copied()
...
impl<R: crate::rng::RandomSource> ShamirSecretSharing<R> {
    ...
    pub fn split_secret(
        ...
        if threshold > total_shares {
            return Err(Error::InvalidParameter("Threshold cannot exceed total shares".into()));
        }
        
        if threshold < 2 {
            return Err(Error::InvalidParameter("Threshold must be at least 2".into()));
        }
        ...
        for share in shares.iter().skip(1) {
            if share.params.identifier != first.params.identifier ||
               ...
               share.value.len() != first.value.len() {
                return Err(Error::InvalidParameter("Inconsistent share parameters".into()));
            }
        }
        
        if shares.len() < first.params.member_threshold as usize {
            return Err(Error::InvalidParameter(
                format!("Need at least {} shares", first.params.member_threshold)
            ));
        }
    ...
    fn evaluate_polynomial(&self, coefficients: &[u8], x: u8) -> u8 {
        ...
        for &coeff in coefficients.iter().rev() {
            result = self.gf256_add(
                self.gf256_mul(result, x),
                coeff
            );
        }
