...
use bitcoin::{
    ScriptBuf, PublicKey, 
    blockdata::{script::Builder, opcodes},
    hashes::Hash,
};
use crate::{Result, Error};

#[cfg(not(feature = "std"))]
use alloc::{vec, vec::Vec, string::{String, ToString}, format, boxed::Box};
#[cfg(feature = "std")]
use std::format;
            ...
            Policy::Threshold(k, subs) => {
                if *k == 0 || *k > subs.len() {
                    return Err(Error::InvalidParameter(
                        format!("Invalid threshold {}/{}", k, subs.len())
                    ));
                }
                
                if *k == 1 {
                    // Any one of them (OR tree)
                    let compiled: Result<Vec<_>> = subs.iter()
                        .map(Self::compile)
                        .collect();
    ...
    pub fn to_script(miniscript: &Miniscript) -> Result<ScriptBuf> {
        let script = match miniscript {
            ...
            Miniscript::Thresh(k, subs) => {
                ...
                if *k == 1 && subs.len() == 2 {
                    // 1-of-2 is just OR
                    let or_ms = Miniscript::Or(
                        Box::new(subs[0].clone()),
                        Box::new(subs[1].clone())
                    );
...
impl PolicyBuilder {
    ...
    pub fn timelocked(pk: PublicKey, locktime: u32) -> Policy {
        Policy::And(
            Box::new(Policy::Key(pk)),
            Box::new(Policy::After(locktime)),
        )
    }
...
pub fn parse_policy(s: &str) -> Result<Policy> {
    ...
    if s.starts_with("pk(") && s.ends_with(')') {
        // Parse pk(pubkey)
        return Err(Error::InvalidParameter("Policy parsing not fully implemented".to_string()));
    }
    ...
    Err(Error::InvalidParameter(format!("Unknown policy format: {}", s)))
}
