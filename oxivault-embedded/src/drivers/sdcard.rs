    ...
    CS: OutputPin,
{
    ...
    pub async fn init(&mut self) -> Result<(), SdError> {
        ...
        let dummy = [0xFF; 10];
        self.spi.write(&dummy).await.map_err(|_| SdError::SpiError)?;
        ...
        loop {
            ...
            let r1 = self.send_command(Command::AppSendOpCond, 0x40000000).await?;
    ...
    pub async fn read_block(&mut self, block_num: u32) -> Result<[u8; 512], SdError> {
        ...
        let r1 = self.send_command(Command::ReadSingleBlock, block_num).await?;
        if r1 != 0x00 {
            self.cs.set_high().await.map_err(|_| SdError::SpiError)?;
        ...
        loop {
            let mut buf = [0xFF];
            self.spi.transfer_in_place(&mut buf).await.map_err(|_| SdError::SpiError)?;
        ...
        self.spi.read(&mut data).await.map_err(|_| SdError::SpiError)?;
        
        // Read and discard 2-byte CRC
        let mut crc = [0xFF; 2];
        self.spi.read(&mut crc).await.map_err(|_| SdError::SpiError)?;
    ...
    pub async fn write_block(&mut self, block_num: u32, data: &[u8; 512]) -> Result<(), SdError> {
        ...
        let r1 = self.send_command(Command::WriteSingleBlock, block_num).await?;
        if r1 != 0x00 {
            self.cs.set_high().await.map_err(|_| SdError::SpiError)?;
            return Err(SdError::InvalidResponse);
        }
        ...
        let token = [0xFE];
        self.spi.write(&token).await.map_err(|_| SdError::SpiError)?;
        ...
        self.spi.transfer_in_place(&mut response).await.map_err(|_| SdError::SpiError)?;
    ...
    async fn send_command(&mut self, cmd: Command, arg: u32) -> Result<u8, SdError> {
        ...
        // Send command
        self.spi.write(&packet).await.map_err(|_| SdError::SpiError)?;
    ...
    async fn send_command_r3(&mut self, cmd: Command, arg: u32) -> Result<(u8, u32), SdError> {
        ...
        let mut ocr = [0u8; 4];
        self.spi.read(&mut ocr).await.map_err(|_| SdError::SpiError)?;
    ...
    CS: OutputPin,
{
    ...
    pub async fn save_backup(&mut self, slot: u8, data: &[u8]) -> Result<(), SdError> {
        ...
        for (i, chunk) in data.chunks(512).enumerate() {
            ...
            self.sd.write_block(base_block + 1 + i as u32, &block).await?;
        }
...
mod tests {
    ...
    impl embedded_hal_async::spi::SpiDevice for MockSpi {
        ...
        async fn read(&mut self, _buf: &mut [u8]) -> Result<(), embedded_hal_async::spi::ErrorKind> {
            Ok(())
        }
        ...
        async fn transfer(&mut self, _read: &mut [u8], _write: &[u8]) -> Result<(), embedded_hal_async::spi::ErrorKind> {
            Ok(())
        }
        
        async fn transfer_in_place(&mut self, _buf: &mut [u8]) -> Result<(), embedded_hal_async::spi::ErrorKind> {
            Ok(())
        }
    }
