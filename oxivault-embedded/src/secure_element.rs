    ...
    I2C: I2c,
{
    ...
    async fn wake_up(&mut self) -> Result<(), SecureElementError> {
        ...
        self.i2c.read(ATECC_I2C_ADDRESS, &mut response).await
    ...
    pub async fn sign(&mut self, slot: u8, hash: &[u8; 32]) -> Result<[u8; 64], SecureElementError> {
        if !self.initialized {
            return Err(SecureElementError::NotInitialized);
        }
        ...
        self.execute_command_with_data(Command::Nonce, &nonce_params, &nonce_data).await?;
    ...
    pub async fn verify(
        ...
        let result = self.execute_command_with_data(
            Command::Verify, 
            &verify_params, 
            &verify_data
        ).await;
    ...
    pub async fn ecdh(&mut self, slot: u8, other_pubkey: &[u8; 64]) -> Result<[u8; 32], SecureElementError> {
        if !self.initialized {
            return Err(SecureElementError::NotInitialized);
        }
        ...
        let response = self.execute_command_with_data(
            Command::ECDH,
            &params,
            other_pubkey
        ).await?;
    ...
    async fn execute_command_with_data(
        ...
        // Send command
        self.i2c.write(ATECC_I2C_ADDRESS, &packet).await
            .map_err(|_| SecureElementError::I2cError)?;
    ...
    /// Read from zone
    async fn read_zone(&mut self, zone: u8, slot: u8, offset: u8) -> Result<[u8; 32], SecureElementError> {
    ...
    /// Write to zone
    async fn write_zone(&mut self, zone: u8, slot: u8, offset: u8, data: &[u8]) -> Result<(), SecureElementError> {
        // Write command parameters
        let addr = ((zone as u16) << 11) | ((slot as u16) << 3) | ((offset as u16 >> 2) & 0x07);
        let params = addr.to_le_bytes();
        
        self.execute_command_with_data(Command::Write, &params, data).await?;
    ...
    I2C: I2c,
{
    ...
    pub async fn sign_bitcoin_transaction(&mut self, hash: &[u8; 32]) -> Result<[u8; 64], SecureElementError> {
    ...
    pub async fn get_entropy(&mut self, size: usize) -> Result<Vec<u8, 256>, SecureElementError> {
        ...
        entropy.resize_default(size).map_err(|_| SecureElementError::InvalidCommand)?;
    ...
    pub async fn derive_session_key(&mut self, peer_pubkey: &[u8; 64]) -> Result<[u8; 32], SecureElementError> {
...
mod tests {
    ...
    impl embedded_hal_async::i2c::I2c for MockI2c {
        async fn read(&mut self, _address: u8, _buffer: &mut [u8]) -> Result<(), embedded_hal_async::i2c::ErrorKind> {
            Ok(())
        }
        
        async fn write(&mut self, _address: u8, _data: &[u8]) -> Result<(), embedded_hal_async::i2c::ErrorKind> {
            Ok(())
        }
