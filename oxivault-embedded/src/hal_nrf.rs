...
use embassy_nrf::{
    gpio::{Input, Output, OutputDrive, Pin, Pull, Level},
    twim::{self, Twim},
    usb::{self, Driver},
    peripherals,
};
...
use crate::{
    hal::{HardwareAbstractionLayer, Display, Button, StorageDevice, SecureElement},
    drivers::ssd1306::{Ssd1306, I2cInterface, Rotation, TextSize},
};
use core::fmt::Write;
use heapless::String;
use defmt::*;
...
impl Display for Nrf52840Display {
    ...
    async fn write_text(&mut self, text: &str, x: u16, y: u16) -> Result<(), crate::hal::Error> {
        self.driver.draw_text(text, x as i32, y as i32, TextSize::Small);
    ...
    async fn show_menu(&mut self, items: &[&str], selected: usize) -> Result<(), crate::hal::Error> {
        self.clear().await?;
        ...
        for (i, item) in items.iter().skip(start_idx).take(items_per_page).enumerate() {
    ...
    CS: embedded_hal_async::digital::OutputPin,
{
    async fn read(&mut self, offset: u32, buffer: &mut [u8]) -> Result<(), crate::hal::Error> {
        ...
        let block_data = self.inner.read_block(block_num).await
            .map_err(|_| crate::hal::Error::StorageError)?;
    ...
    async fn write(&mut self, offset: u32, data: &[u8]) -> Result<(), crate::hal::Error> {
        ...
        if data.len() <= 512 {
            ...
            self.inner.write_block(block_num, &block_data).await
                .map_err(|_| crate::hal::Error::StorageError)?;
        }
        Ok(())
    }
    ...
    I2C: embedded_hal_async::i2c::I2c,
{
    pub async fn new(i2c: I2C) -> Result<Self, crate::hal::Error> {
        ...
        let wallet = SecureWallet::new(driver).await
    ...
    I2C: embedded_hal_async::i2c::I2c,
{
    async fn random(&mut self, buffer: &mut [u8]) -> Result<(), crate::hal::Error> {
        let entropy = self.inner.get_entropy(buffer.len()).await
            .map_err(|_| crate::hal::Error::SecureElementError)?;
    ...
    async fn sign(&mut self, message: &[u8]) -> Result<[u8; 64], crate::hal::Error> {
        ...
        self.inner.sign_bitcoin_transaction(&hash).await
            .map_err(|_| crate::hal::Error::SecureElementError)
    }
    
    async fn verify(&mut self, message: &[u8], signature: &[u8]) -> Result<bool, crate::hal::Error> {
...
pub fn init_usb(p: peripherals::USBD) -> Driver<'static, peripherals::USBD, usb::vbus_detect::HardwareVbusDetect> {
    // Create USB driver
    Driver::new(p, usb::Irqs, usb::vbus_detect::HardwareVbusDetect::new(usb::vbus_detect::Irqs))
}
